...
echo "===== Шаг 2: вырезаем паузы (auto-editor, с ограничением по времени) ====="
# ВРЕМЕННО: auto-editor делает полное пересжатие и на длинном видео бывает
# медленным/зависает у 100%. Ограничиваем по времени (8 минут) и, если не успел
# или упал, берём исходник без вырезки пауз — чтобы конвейер точно прошёл дальше.
# Правильное решение (вырезка списком кусков внутри одной команды ffmpeg) — TODO.
if timeout 480 auto-editor "$RAW" --margin 0.2sec --no-open --output "$CUT"; then
  echo "Паузы вырезаны."
else
  echo "auto-editor не уложился/завис — беру исходник без вырезки пауз (временно)."
  cp "$RAW" "$CUT"
fi
